...
.item--color1::before {
  background: repeat-x 100% 0/50% 100%
    radial-gradient(circle at 50% 0, transparent 25%, #7f5fff 65%);
}
...
.noise {
  ...
  width: 500px;
  height: 500px;
  background: radial-gradient(circle at 50% 50%, black, rgba(0, 0, 0, 0)),
  ...
  animation: distort 5s ease-in-out infinite alternate; /* Animation de déformation */
}

@keyframes distort {
  from {
    transform: skew(0deg); /* Début de l'animation : aucune déformation */
  }
  to {
    transform: skew(
      20deg,
      30deg
    ); /* Fin de l'animation : déformation avec un angle de 20 degrés selon l'axe X et 30 degrés selon l'axe Y */
  }
}
